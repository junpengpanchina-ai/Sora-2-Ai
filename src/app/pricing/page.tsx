...
import { SUBSCRIPTION_PLANS, SubscriptionPlan } from '@/lib/plans'
...
export default function PricingPage() {
  ...
  const handleSubscribe = async (plan: SubscriptionPlan) => {
    ...
    // 如果没有直接链接，使用API创建会话
    // planConfig 一定存在，因为 plan 来自 SUBSCRIPTION_PLANS 的键
    const config = planConfig as (typeof SUBSCRIPTION_PLANS)[SubscriptionPlan]
    if (!config) return
        ...
        <div className="mt-20">
          <div className="text-center mb-16">
            <h3 className="text-3xl font-bold text-gray-900 mb-4">
              {t.pricing('testimonials.title') || '用户评价'}
            </h3>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              {t.pricing('testimonials.subtitle') || '听听我们的用户怎么说'}
            </p>
          </div>
          ...
          <div className="mt-16 bg-gradient-to-r from-blue-50 to-purple-50 rounded-3xl p-8">
            <div className="text-center mb-8">
              <h4 className="text-2xl font-bold text-gray-900 mb-2">{t.pricing('stats.title') || '成功案例'}</h4>
              <p className="text-gray-600">{t.pricing('stats.subtitle') || '我们的数据说明一切'}</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {t.pricing('faq.cancel.title') || '如何取消订阅？'}
              </h3>
              <p className="text-gray-600">
                {t.pricing('faq.cancel.answer') || '您可以随时在账户设置中取消订阅，不会产生任何额外费用。'}
              </p>
            </div>
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {t.pricing('faq.payment.title') || '支持哪些支付方式？'}
              </h3>
              <p className="text-gray-600">
                {t.pricing('faq.payment.answer') || '我们支持所有主流的信用卡和借记卡，通过 Stripe 安全支付。'}
              </p>
            </div>
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {t.pricing('faq.generation.title') || '视频生成需要多长时间？'}
              </h3>
              <p className="text-gray-600">
                {t.pricing('faq.generation.answer') || '根据视频长度和复杂度，通常在 2-5 分钟内完成生成。'}
              </p>
            </div>
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-2">
                {t.pricing('faq.api.title') || '是否提供 API 接口？'}
              </h3>
              <p className="text-gray-600">
                {t.pricing('faq.api.answer') || 'Teams 计划包含完整的 API 访问权限，可以集成到您的工作流中。'}
              </p>
            </div>
          </div>
        </div>
